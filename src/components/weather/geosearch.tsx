    ...
    <div className="m-20 flex w-fit flex-col justify-center gap-4 rounded-md border-2 p-3">
        ...
        <div className="scroll-y-auto flex max-h-64 flex-col items-start gap-2 overflow-y-scroll">
          {data.map((element) => (
            <p
              ...
              className={`w-full border-b border-gray-200 py-1 hover:bg-gray-200 pl-1 cursor-pointer ${selectedLocation?.lat === element.lat && selectedLocation?.lng === element.lng ? "bg-gray-300" : ""}`}
            >
      ...
      {selectedLocation && (
        <div className="flex flex-row justify-between gap-2">
          <button className="w-full rounded-md bg-green-500 p-2 cursor-pointer text-white hover:bg-green-600">
